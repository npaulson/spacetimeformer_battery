...
if __name__ == '__main__':
    ...
    feature_names = ['Q_d', 'E_d', 'Q_eff', 'E_eff', 'ohmic']
    ...
    for ii in range(0, n_tst, 1):
        ...
        def life_pred_f(Q_val, Qs, cycs):
            ...
            if np.isnan(cyc_val):                
                ...
                lvlgtp = lvl > np.max(Qs)  # if EOL Q > all predicted Qs

                if lvlgtp:
                    lr_y = cycs - cycs[0]
                    weights = np.linspace(1, 0.01, len(cycs))
                else:
                    lr_y = cycs - cycs[-1]
                ...
                if len(cycs) > lr_cycs:
                    if lvlgtp:
                        lr_x = lr_x[:lr_cycs, :]
                        lr_y = lr_y[:lr_cycs]
                        weights = np.linspace(1, 0.01, lr_cycs)
                    else:
                        lr_x = lr_x[-lr_cycs:, :]
                        lr_y = lr_y[-lr_cycs:]
                ...
                if lvlgtp:
                    cyc_val = slope*(Q_val - Qs[0]) + cycs[0]
                else:
        ...
        if out and np.abs(cyc_model-cyc_exp) > 100:
            ...
            if not np.isnan(slope_model):
                ...
                lvlgtp = lvl > np.max(pred[:, 0])
                if lvlgtp:
                    cycles_m = slope_model*(caps-pred[0, 0]) + cycles_t[0]
                else:
    ...
    pd.DataFrame({'exp':x_data, 'pred':y_data, 'cathode':inp_lab['category'].values}).to_csv(
        f'exp_pred_{pct_lvl}_{cset}.csv', index=False)
    ...
    for kk, cat in enumerate(categoriesu):
        ...
        eol_metrics['MAE'].append(np.nanmean(np.abs(x_data[sel]-y_data[sel])))
        eol_metrics['RMSE'].append(np.sqrt(np.nanmean((x_data[sel]-y_data[sel])**2)))

        label = f'{cat}'

        plt.plot(x_data[sel], y_data[sel], color=cs[kk], mfc='none',
            marker=ms[kk], ls='', ms=5, alpha=0.99, label=label)
    ...
    index = np.append(categoriesu, 'All')
    ...
    if len(x_data) <= 1:
        ...
        print(np.nansum((x_data - y_data)**2), np.nansum((x_data - np.nanmean(x_data))**2))
